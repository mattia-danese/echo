...
export interface PlatformStatic {
  ...
  refreshTokens(request: RefreshTokensRequest): Promise<{
    ok: boolean;
  ...
  searchTracks(request: SearchTracksRequest): Promise<{
    ok: boolean;
...
export abstract class Platform<_T extends PlatformStatic = PlatformStatic> {
...
export function isPlatformClass<T extends PlatformStatic>(cls: T): cls is T {
  return (
    typeof cls.refreshTokens === "function" &&
...
export type PlatformClass<T extends PlatformStatic = PlatformStatic> = new (
  ...args: unknown[]
) => Platform<T> & T;
